...
const ChatContainer = () => {
  ...
  const handleSendImage =async(e)=>{
    ...
    if(!file || !file.type.startsWith("image/")){
      toast.error("Select a proper image type");
      return;
    }
    ...
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      ...
      <div className='flex items-center gap-3 py-3 mx-4 border-b border-stone-500'>
        ...
        <p className='flex-1 text-lg text-white flex items-center gap-2'>
          ...
          {/* find how this shit works(making the other one flex makes the circle appear well) */}
        </p>
      ...
      {/* Input area */}
        ...
        <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full'>
          <input onChange={(e)=>setInput(e.target.value)} value={input} onKeyDown={(e)=>{
            e.key==="Enter" ? handleSendMessage(e) : null}} type="text" placeholder='send a message' className='flex-1 text-sm p-3 border-none rounded-lg outline-none text-white placeholder-gray-400' />
